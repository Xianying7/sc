...
eval(str_replace('name_host',explode(".","tokenmix.pro")[0],str_replace('example',"tokenmix.pro",'const host="https://example/",sc="name_host",cookie_only="cookie_example";')));
...
$r = base_run("https://tokenmix.pro/infos/dashboard_info",1);
...
while (true) {
    ...
    $r = base_run("https://tokenmix.pro/infos/auth_page_info", $data, 1);
    ...
    if ($r1["notif"]) {
        print h . $r1["notif"];
        r();
        $r3 = base_run("https://tokenmix.pro/infos/dashboard_info", 1);
...
function h_tk($json = 0) {
    ...
    $header[] = 'referer: https://tokenmix.pro/dashboard/sl';
    $header[] = 'cookie: '.$u_c;
    return $header;
}
...
function bypass_shortlinks_tokenmix($r) {
    ...
    $js = base_run("https://tokenmix.pro/user/generateSl", $data, 1)["json"];
